...
router.route('/subject/questions/:code').get((request, response) => {
    let code = request.params.code;
    Subject_1.default.findOne({ 'info.code': code }, { exams: 1 }, (err, res) => {
        if (err)
            console.log(err);
        if (res)
            response.json(res.exams.questions);
        else
            response.json([]);
    });
});
router.route('/subject/solutions/:code').get((request, response) => {
    ...
    Subject_1.default.findOne({ 'info.code': code }, { exams: 1 }, (err, res) => {
        if (err)
            console.log(err);
        if (res)
            response.json(res.exams.solutions);
        else
            response.json([]);
    });
});
...
router.route('/subject/applicant/:code').get((request, response) => {
    let code = request.params.code;
    Subject_1.default.findOne({ 'info.code': code }, { applicants: 1 }, (err, res) => {
        if (err)
            console.log(err);
        if (res)
            response.json(res);
